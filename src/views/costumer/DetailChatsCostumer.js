...
import { getListChatDetailVendor, sendMessageVendor } from '../../services/enpoint/costumer';

export default function DetailChatsCostumer() {
    ...
    const getListChats = () => {
        getListChatDetailVendor(token, conversation_id)
        .then((result) => {
            const resp = result
            // console.log(resp);
            let arryMes = []
    ...
    const handleSend = async (event) => {
        ...
        sendMessageVendor(token, conversation_id, dataRecipient)
        .then((result) => {
            getListChats()
        }).catch((err) => {
            console.log(err);
    ...
    return (
        <>
            <section className="dark:bg-gray-900 relative w-full">
                <div className="container mx-auto">
                    ...
                    <div className="flex flex-row w-full overflow-x-hidden">
                        <div className="flex flex-col flex-auto h-full p-5">
                            <div className='flex bg-gray-100 pt-2 pl-3 justify-between'>
                                <img
                                    src={InfoRecipient?.recipient?.avatar?.src}
                                    alt="..."
                                    class="h-16 w-16 rounded-2xl"
                                />
                                <Link to="/costumer/list-chats">
                                    <button className='p-2 bg-blueGray-600 rounded-lg text-white mr-3'>
                                        Kembali
                                    </button>
                                </Link>
                            </div>
